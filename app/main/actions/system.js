...
global.st.system.status((error, payload) => {
  if(error){
    ...
    dispatch({
      type: 'SYSTEM_STATUS_GET_SUCCESS',
      payload,
    })
  }
})
...
global.st.system.connections().then(({total, connections}) => {
  dispatch({
    type: 'BANDWITH_TOTAL_GET_SUCCESS',
    payload: {
      upBytes: total.outBytesTotal,
      downBytes: total.inBytesTotal,
    },
  })
  dispatch({
    type: 'CONNECTIONS_GET_SUCCESS',
    payload: connections,
  })
}).catch(error => {
  dispatch({
    type: 'CONNECTIONS_GET_FAILURE',
    error,
  })
})
...
global.st.system.getConfig().then(({devices, folders, options, gui}) => {
  ...
  dispatch({
    ...
    payload: devices.filter(({deviceID}) => deviceID != myID),
  })
  dispatch({
    type: 'GUI_PREFERENCES_GET_SUCCESS',
    payload: gui,
  })
  dispatch({
    type: 'PREFERENCES_GET_SUCCESS',
    payload: options,
  })
}).catch(error => {
  dispatch({
    type: 'SERVICE_CONFIG_GET_FAILURE',
    error,
  })
})
